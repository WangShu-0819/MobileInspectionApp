...
## 9. 后续业务集成边界（2026-09-02）

在本报告的软件解码链验收之外，新工程后续接入的 DPM 业务行为已明确为：

- 现场采集顶部“扫一扫”入口保持不变，继续使用本报告所述实时 DPM 解码链和唯一 `CameraController`。
- 扫码命中 `PartEntity.dpmCode` 后，只切换已有零件及其有序模板视角，并回到现场采集的 View 1/N；不在现场扫码流程中新建零件或录入未知码。
- 未绑定码只提示先在“我的 > 模板配置”中绑定。
- 模板配置按零件提供扫码绑定/更换 DPM 码，并拒绝覆盖其他零件已经使用的码。
- 上述业务路由和数据库查询/更新已接入新工程源码；本报告第 8 节的历史 APK 证据不等同于这次业务扩展的真机验收。扩展验证以新 APK 为准，并必须重新执行包名门禁。
