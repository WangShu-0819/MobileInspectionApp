...
1. [ ] 在 `PartListScreen` 增加真实“新建零件”入口和创建对话框。
2. [ ] 校验零件 ID、名称和重复 ID，通过现有 Repository 写入 `PartEntity`；允许创建 0 View 的空零件。
3. [ ] 创建成功后导航到对应 `PartDetailScreen`，正确显示零件信息和 0 View 空状态。
4. [ ] 验证从该详情页继续导入多张图片时全部写入当前 `partId`，且已有导入/拍摄流程不回归。
5. [ ] 补充 JVM 自动化测试，执行三条 Gradle 命令，更新任务清单和整改报告；本轮不执行 adb。
...
- [ ] 模板配置零件列表可不依赖图片直接新建零件
- [ ] 新建零件校验 ID/名称/重复项并真实写入数据库
- [ ] 创建成功后进入正确 PartDetail，允许 0 View 空状态
- [ ] 后续多图导入全部关联当前 `partId`
- [ ] 原有导入、拍摄、重拍、删除、排序和 ROI 回归通过
